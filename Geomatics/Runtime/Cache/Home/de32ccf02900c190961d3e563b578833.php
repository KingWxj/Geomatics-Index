...
<body>
	...
	<?php if(is_array($brother)): foreach($brother as $key=>$vo): ?><li><a href="<?php echo U('Show/showNav',array('id'=>$vo['id']));?>"><?php echo ($vo['title']); ?></a></li><?php endforeach; endif; ?>	
	...
	<p><?php echo ($title); ?>列表</p>

			<ul class="main_new_list">
				<!--右侧的主要内容列表-->
				
	<?php if(is_array($list)): foreach($list as $key=>$vo): ?><a href="<?php echo U('Show/showNavListArt',array('id'=>$vo['id']));?>"><li style="padding: 5px;cursor: pointer;"><span><?php echo (mb_substr($vo['title'] ,0,35,utf8)); ?></span><span style="float: right;"><?php echo (mb_substr($vo['date'] ,0,10,utf8)); ?></span></li></a><?php endforeach; endif; ?>
