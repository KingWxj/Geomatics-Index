...
<html lang="zh-CN">
	...
	<script src="/Project/Public/Admin/js/Admin_index.js" type="text/javascript" charset="utf-8"></script>
	...
	<th>是否可用</th>
	<th>操作</th>
	<?php if(is_array($adminList)): foreach($adminList as $key=>$list): ?><tr>
			<td><?php echo ($list['id']); ?></td>
			<td><?php echo ($list['name']); ?></td>
			<td><?php if($list["level"] == 0): ?>学校
			...
			<td><?php if($list['avaliable'] == 0): ?>已禁用
				    <?php else: ?>
				    已启用<?php endif; ?></td>
			<td><a href="javascript:void(0)" onclick="ajaxSwapAvaliable('<?php echo U('Admin/swapAvaliable');?>','<?php echo ($list['id']); ?>')">
				<!--判断账户的启用状态，输出不同的按钮-->
				<?php if($list['avaliable'] == 1): ?>[禁用]
				    <?php else: ?>
				    [启用]<?php endif; ?>
			</a> 
				<a href="javascript:void(0)" onclick="ajaxDelAdmin('<?php echo U('Admin/delAdmin');?>','<?php echo ($list['id']); ?>')">
					[删除]
				</a></td>
