...
<html lang="zh-CN">
	...
	<body>
		...
		<?php if(is_array($info)): foreach($info as $key=>$arr): ?><table border="1px" cellspacing="0" width="95%" style="margin: 5px auto;">
		<tr>
			<th colspan="3" align="center" style="font-size: 18px;"><?php echo ($arr[0]['root']); ?></th>
		</tr>
		<?php if(is_array($arr)): foreach($arr as $key=>$vo): ?><tr>
						<td align="center" width="70%"><?php echo ($vo['title']); ?></td>
						<td align="center" width="10%">类型：<?php echo ($vo['type']); ?></td>
						<td align="center">
							<?php switch($vo['type']): case "文章": ?><a href="<?php echo U('Nav/mgrNavArt',array('id'=>$vo['id']));?>">[修改文章]</a><?php break;?>
								<?php case "列表": ?><a href="<?php echo U('Nav/mgrNavList',array('id'=>$vo['id']));?>">[管理列表]</a><?php break;?>
								<?php case "链接": ?><a href="<?php echo U('Nav/mgrNavLink',array('id'=>$vo['id']));?>">[修改链接]</a><?php break; endswitch;?>	
						</td>
