...
<html lang="zh-CN">
	...
	<?php if($_COOKIE['adminLevel']== 3): ?>对不起，只有学校管理员和学院管理员有权限添加账户
		<?php elseif($_COOKIE['adminLevel']== 2): ?>
		对不起，只有学校管理员和学院管理员有权限添加账户
		<?php else: ?>
		<form action="<?php echo U('Admin/runAddAdmin');?>" method="post">
			...
			<select name="level">
				<?php if($_COOKIE['adminLevel']== 0): ?><option value="0">学校</option>
					<option value="1">学院</option>
					<option value="2">班主任</option>
					<option value="3">辅导员</option>
					<?php elseif($_COOKIE['adminLevel']== 1): ?>
					<option value="1">学院</option>
					<option value="2">班主任</option>
					<option value="3">辅导员</option><?php endif; ?>
			</select>
			<br />
