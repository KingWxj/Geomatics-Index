...
<html lang="zh-CN">
	...
	<form action="<?php echo U('Nav/runUpdateNav');?>" method="post" style="margin: 10px 0 10px 20px;">
		<!--设置隐藏表单是为了，将修改项的id传过来，这样runUpdateNav控制器中也能再次接收到该id值，id值要作为查询数据的条件-->
		<input type="hidden" name="hid_id" value="<?php echo ($info['id']); ?>" />
		<label for="info">信息：</label><input type="text" name="info" id="info" value="<?php echo ($info['info']); ?>" style="width: 500px; padding: 5px; margin: 5px;" />
	<input type="submit" value="提交修改" style="padding: 5px;"/>
