...
<html lang="zh-CN">
	...
	<form action="<?php echo U('Consult/noName');?>" method="post" style="margin: 10px 0 0 20px;">
		是否启用匿名发表
		<?php if($status['value'] == '1'): ?><input type="radio" name="status" id="yes" value="1" checked="checked" />
			<label for="yes">启用</label>
			<input type="radio" name="status" id="no" value="0" />
			<label for="no">禁用</label>	
		<?php elseif($status['value'] == '0'): ?>
			<input type="radio" name="status" id="yes" value="1" />
			<label for="yes">启用</label>
			<input type="radio" name="status" id="no" value="0" checked="checked" />
			<label for="no">禁用</label><?php endif; ?>		
	...
	<table border="1px" cellspacing="0" width="100%">
		...
		<?php if(is_array($allTheme)): foreach($allTheme as $key=>$vo): ?><tr>
				<td align="center"><a href="<?php echo U('Consult/showFull',array('id'=>$vo['id']));?>"><?php echo (htmlspecialchars($vo['title'] )); ?></a></td>
				<td align="center"><?php echo ($vo['uid']); ?></td>
				<td align="center"><?php echo ($vo['date']); ?></td>
				<td align="center"><a href="<?php echo U('Consult/delData',array('id'=>$vo['id']));?>">[删除]</a></td>
